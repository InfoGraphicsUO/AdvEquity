...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (this.id === 'agg-selectState'){
    window.aggLevel = 'state';
        ...
        $('#race-selectBlk').click() // trigger returning to US view wiht Opportunity Estimate for Black Students
       ...
       // display district level data 
        ...
        getDistrictData('all').then(districtData => {
          fillDistrictMap(map, districtData, 'all', 'all', 'ENR_AP_GAP_BL');
          if (typeof updateControlStates === 'function') updateControlStates();
        });
      }
    })
  ...
  fullExtentButton.addEventListener('click', () => {
    ...
    hideGraphs();
  $('#race-selectBlk').click(); // trigger returning to US view Opportunity Estimate for Black Students
...
function drawMiniChart(canvasId, years, series, colors, yLabel = '') {
  ...
  if (!allVals.length) return; // nothing to draw
  let min = Math.min(...allVals);
  let max = Math.max(...allVals);
  if (min === max) {
    // avoid division by zero when series are flat
    min = min - 1;
    max = max + 1;
  }
...
// Generic simple legend for District/State/National style sparklines
function drawSimpleLegend(containerId, labelMap, colors = {}, series = {}) {
  ...
  Object.keys(labelMap).forEach(key => {
    if (!series[key]) return; // don't show legend entry if series not present
    const span = document.createElement('span');
    span.style.display = 'inline-block';
